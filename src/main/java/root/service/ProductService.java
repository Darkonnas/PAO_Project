...
public final class ProductService {
    ...
    public Set<Product> getProducts() {
        LogService.getInstance().log("Requested products", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProducts();
    }
    
    public int addProduct(final Product p) {
        LogService.getInstance().log("Added a product", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.add(p);
    }
    
    public int removeProduct(final int id) {
        LogService.getInstance().log("Removed a product", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.remove(id);
    }
    
    public Product getProductById(final int id) {
        LogService.getInstance().log("Requested product by id", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProductById(id);
    }
    
    public Set<Product> getProductsByCategoryId(final int categoryId) {
        LogService.getInstance().log("Requested products by categoryId", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProductsByCategoryId(categoryId);
    }
    
    public int setProductCategoryId(final int id, final int categoryId) {
        LogService.getInstance().log("Set product categoryId", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.setProductCategoryId(id, categoryId);
    }
    
    public Set<Product> getProductsByName(final String name) {
        LogService.getInstance().log("Requested products by name", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProductsByName(name);
    }
    
    public int setProductName(final int id, final String name) {
        LogService.getInstance().log("Set product name", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.setProductName(id, name);
    }
    
    public Set<Product> getProductsByPrice(final float price) {
        LogService.getInstance().log("Requested products by price", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProductsByPrice(price);
    }
    
    public int setProductPrice(final int id, final float price) {
        LogService.getInstance().log("Set product price", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
    ...
    public Set<Product> getProductsByQuantity(final int quantity) {
        LogService.getInstance().log("Requested products by quantity", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
        return productRepository.getProductsByQuantity(quantity);
    }
    
    public int setProductQuantity(final int id, final int quantity) {
        LogService.getInstance().log("Set product quantity", new Timestamp(System.currentTimeMillis()), Thread.currentThread().getName());
